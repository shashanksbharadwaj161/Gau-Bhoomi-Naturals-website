                  ...
                  {items.map((it) => (
                    <li key={it.id} className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-lg overflow-hidden bg-primary-500 flex-shrink-0 flex items-center justify-center relative">
                        {it.image ? <img src={it.image} alt={it.name} onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = PRODUCT_IMAGE_FALLBACK }} className="w-full h-full object-cover" /> : <span className="text-gold-400 text-[9px] font-display">GBN</span>}
